...
def gerenciar_mural(request):
    ...
    if "edit_mural_item" in request.POST:
        if not request.user.has_perm("mural.ger_mural"):
            messages.error(
                request, "Você não possui permissão para editar itens no mural.")
            return redirect("mural:mural_gerenciamento")
        ...
        try:
            ...
            messages.error(
                request,
                f"Erro ao enviar novo arquivo/imagem para o Firebase. Erro: {e}"
            )
            return redirect("mural:mural_gerenciamento")
        ...
        try:
            edit_url = f"{MURAL_API_URL}/v1/items/update-item/{edit_item_id}"
            ...
            if isinstance(edit_resp, dict) and "detail" in edit_resp:
    ...
    if "disable_item" in request.POST:
        if not request.user.has_perm("mural.ger_mural"):
            messages.error(
                request, "Você não possui permissão para desabilitar itens no mural.")
            return redirect("mural:mural_gerenciamento")

        item_id = request.POST.get("disable_item_id")

        try:
            disable_url = f"{MURAL_API_URL}/v1/items/disable-item/{item_id}"
            ...
            if isinstance(disable_resp, dict) and "detail" in disable_resp:
    ...
    if "mark_read_item" in request.POST:
        ...
        try:
            mark_read_url = f"{MURAL_API_URL}/v1/item-reads/?manualconfirmation=true"
            ...
            if isinstance(mark_read_resp, dict) and "detail" in mark_read_resp:
