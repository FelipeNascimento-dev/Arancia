...
| 2026-06-19 | CRM | Fase 2–3: gates Django (`has_perm`); bundles agregados default; `/me/context` só menu | `CRM_USE_AGGREGATED_ENDPOINTS=true`; Kanban 2–3 calls; SLA Kanban <3s / dashboard <2s |
| 2026-06-19 | CRM | Fase 1 auth Bearer BFF: `Authorization: Bearer {CRM_INTERNAL_API_SECRET}` + `X-Acting-User`; flag `CRM_BFF_AUTH_MODE` (`bearer` default, rollback `legacy_basic`) | `crm_api/context.py`; jobs Celery usam `CRM_SERVICE_USERNAME` como acting user; senha na sessão mantida só para rollback |
