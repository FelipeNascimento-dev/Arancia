...
from ..viewsV2.view_enviar_cotacaoV2 import send_quotesV2
...
def reverse_createV2(request):
    ...
    if request.method == "POST" and form.is_valid() and "finalizar_rom" in request.POST:
        if request.method == "POST" and form.is_valid() and "finalizar_rom" in request.POST:
            from_location_id = request.session.get("reverse_origin_id")
            to_location_id = request.session.get("reverse_destination_id")

            if from_location_id in [None, "", "None"] or to_location_id in [None, "", "None"]:
                messages.error(
                    request, "Origem e destino não foram definidos na criação do romaneio.")
                return redirect("logistica:reverse_createV2")
            ...
            try:
                ...
                messages.error(request, "Origem ou destino inválidos.")
                return redirect("logistica:reverse_createV2")
        ...
        return redirect("logistica:consultar_romaneioV2")
...
def cancel_btn(request, id):
    ...
    return render(request, "logistica/templatesV2/criar_reversaV2.html", {
        "form": form,
        "botao_texto": "Inserir",
        "site_title": "Reversa",
        "result": request.session.get("result", None),
