...
from logistica.forms import CreateGAIForm
...
def skill_transport(request):
    ...
    if request.method == "POST" and "create_group" in request.POST:
        ...
        if form.is_valid():
            ...
            _group = Group.objects.get(name='arancia_TRANSPORT')
            ...
            messages.success(
                request,
                f"Cliente {grupo.nome} criado com sucesso!"
            )

            return redirect("transportes:skill_transport")
    ...
    if request.method == "POST" and "edit_group" in request.POST:
        ...
        messages.success(
            request,
            f"Cliente {grupo.nome} atualizado com sucesso!"
        )
    ...
    if request.method == "POST" and "add_user" in request.POST and selected_group:
        ...
        return redirect(
            f"{request.path}?group_id={selected_group.id}"
        )
    ...
    return render(
        request, "transportes/transportes/criar_transportadora.html", context,
    )
