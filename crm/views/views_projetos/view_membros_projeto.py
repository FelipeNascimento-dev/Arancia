...
from crm.forms import ProjectMemberForm
...
def membros_projeto(request, project_id):
    ...
    if request.method == "POST" and (
        ...
        if not request.user.has_perm("crm.manage_project_members"):
            ...
            return redirect("crm:membros_projeto", project_id=project_id)
    ...
    return render(
        request,
        "crm/templates_projetos/membros_projeto.html",
        {
            "site_title": f"CRM — Membros — {projeto.get('name') or project_id}",
            "projeto": projeto,
