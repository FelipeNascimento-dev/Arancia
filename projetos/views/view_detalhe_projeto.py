...
def detalhe_projeto(request, project_id):
    ...
    try:
        raw_projetos, _ = projects_service.list_projects(client, limit=100, q=q or None)
        projetos = [enrich_project(p) for p in raw_projetos]
    except CrmApiError:
        projetos = []

    return render(
        request,
        "projetos/templates_projetos/detalhe_projeto.html",
        {
            "site_title": f"Projetos — {projeto.get('name') or projeto.get('nome') or project_id}",
            "projeto": projeto,
