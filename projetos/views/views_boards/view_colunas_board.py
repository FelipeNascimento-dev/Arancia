...
def colunas_board(request, board_id):
    ...
    if request.method == "POST" and "add_column" in request.POST:
        if not request.user.has_perm("crm.manage_board_columns"):
            ...
            return redirect("projetos:colunas_board", board_id=board_id)
    ...
    return render(
        request,
        "projetos/templates_boards/colunas_board.html",
        {
            "site_title": f"Projetos — Colunas — {board.get('name') or board_id}",
            "board": board,
            "board_id": board_id,
            "columns": columns,
